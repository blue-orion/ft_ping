...
void	print_reply_result(reply_t *reply, int cc, struct timespec *recv_time) {
	...
	printf("%d btyes from %s: icmp_seq=%d ttl=%d time=%.3f ms\n", cc, source, seq, ttl, rtt);
}
...
void	print_statistics(ping_rts_t *rts, statistic_t *stat) {
	...
	double runtime = (end.tv_sec - stat->st.tv_sec) * 1000.0;
	runtime += (end.tv_nsec - stat->st.tv_nsec) / 1000000.0;
